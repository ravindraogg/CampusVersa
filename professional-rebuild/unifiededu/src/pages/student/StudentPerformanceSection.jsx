...
const StudentPerformanceSection = ({ student }) => {
  ...
  // Sort semesters numerically (e.g. 1, 2, 3) to ensure correct graph order
      ...
      <div className="w-full mt-6 select-none">
        <div className="relative w-full aspect-[3/1] max-h-60">
          <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
            ...
            {/* Line */}
            <path d={pathData} fill="none" stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-md"/>

            {/* Points */}
            {points.map((p, i) => (
              <g key={i} className="group">
                <circle cx={p.x} cy={p.y} r="6" fill="#fff" stroke={color} strokeWidth="2" className="transition-all group-hover:r-8"/>
