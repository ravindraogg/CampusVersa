...
export default function NaacPage({ authFetch, theme, institute, pushToast }) {
  ...
  // 2. Load AI Data (Slow - Background)
  const loadAiData = async () => {
    // Only fetch if we don't have suggestions yet or want to refresh
    setAiLoading(true);
    try {
      const res = await authFetch("/institute/naac?ai=1", { method: "GET" });
      const json = await res.json();
      
      // Update state with new data containing AI suggestions
      setData(json);
    } catch (err) {
      console.error("AI Fetch Error", err);
      // We don't block the user here, just log it. 
      // The basic data is already visible.
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleSubmitProof = async () => {
    if (!formText) return pushToast({ type: "error", message: "Please add details" });
    ...
    try {
      ...
      if (res.ok) {
        pushToast({ type: "success", message: "Proof submitted for verification" });
        setSelectedCriteria(null);
        setFormText("");
        
        // Optimistic update or reload basic data
        loadBasicData(); 
      } else {
  ...
  // Initial Full Page Loader (Only for first fraction of a second)
  if (loading) return (
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in duration-500">
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl mt-4 md:mt-0">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
              activeTab === "submission" ? "bg-white text-gray-800 shadow-sm" : "text-gray-500 hover:text-gray-700"
            }`}
          >
            <div className="flex items-center gap-2">
              <FileText className="w-4 h-4" /> Institute Submission
            </div>
          </button>
          <button
          ...
          >
            <div className="flex items-center gap-2">
              {aiLoading ? <Loader2 className="w-3 h-3 animate-spin"/> : <Sparkles className="w-4 h-4" />} 
              AI Strategy
            </div>
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 pr-2 space-y-4 pb-4 custom-scrollbar">
        ...
        {/* --- TAB 1: INSTITUTE SUBMISSION --- */}
        {activeTab === "submission" && (
          <div className="grid grid-cols-1 gap-4">
            {data?.criteria?.map((item) => (
              <div 
                key={item.id} 
                className={`group bg-white p-5 rounded-2xl border transition-all hover:shadow-md ${
                  item.status === 'Rejected' ? 'border-red-200' : 'border-gray-100'
                }`}
              >
                <div className="flex justify-between items-start">
                  <div className="flex gap-4 w-full">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      item.status === 'Verified' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-600'
                    }`}>
                    ...
                    <div className="flex-1">
                      ...
                      {item.submissionText && (
                        <p className="text-sm text-gray-500 mt-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                          <span className="font-semibold text-gray-700 block text-xs mb-1 uppercase">Your Submission:</span>
                          "{item.submissionText}"
                        </p>
                      )}
                      ...
                      {item.adminComments && (
                        <div className="mt-3 text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-100 flex gap-2 items-start">
                          ...
                          <div>
                            <span className="font-bold block text-xs uppercase mb-1">Admin Feedback:</span>
                            {item.adminComments}
                          </div>
                        </div>
                      )}
                  ...
                  {item.status !== 'Verified' && (
                    <button
                      onClick={() => {
                        ...
                        setFormText(item.submissionText || "");
                      }}
                      className="ml-4 px-4 py-2 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-gray-800 transition shadow-lg shrink-0 whitespace-nowrap"
                    >
        ...
        {activeTab === "ai" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Loading Skeleton for AI Tab */}
            {aiLoading && !data?.criteria?.[0]?.suggestion && (
              <>
                {[1,2,3,4].map(i => (
                  <div key={i} className="bg-gray-50 p-6 rounded-3xl border border-gray-100 animate-pulse h-48">
            ...
            {!aiLoading && data?.criteria?.map((item) => (
              <div key={item.id} className="bg-gradient-to-br from-indigo-50 to-white p-6 rounded-3xl border border-indigo-100 relative overflow-hidden animate-in fade-in zoom-in-95 duration-500">
                <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-100 rounded-full -mr-10 -mt-10 opacity-50 pointer-events-none" />
                
                <div className="flex items-center gap-2 mb-3 text-indigo-600">
                  <BrainCircuit className="w-5 h-5" />
                  <span className="font-bold text-xs uppercase tracking-wider">AI Suggestion</span>
                </div>
                
                <h4 className="font-bold text-gray-800 mb-2">{item.name}</h4>
                <p className="text-gray-600 text-sm leading-relaxed">
                  {item.suggestion || "AI analysis pending. Strategy generation in progress..."}
                </p>

                <div className="mt-4 pt-4 border-t border-indigo-100 flex justify-between items-center">
                  <span className="text-xs text-indigo-400 font-medium">Based on 2024 Guidelines</span>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none h-32"
                  placeholder="Describe your compliance, mention metric numbers..."
                />
              </div>

              <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:border-gray-400 transition-colors cursor-pointer bg-gray-50">
                <Upload className="w-8 h-8 text-gray-400 mb-2" />
                <p className="text-sm font-medium text-gray-600">Upload Supporting Documents</p>
